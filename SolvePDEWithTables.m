...
function [V,HoldingProfit,BuyingProfit,SellingProfit] = ...
    SolvePDEWithTables(M,U,alpha,beta,kappa,sigma,Xmax,Xmin,Qmax,Qmin,NumX,NumQ,RegionIndicater,BuyingType,BuyingCostPara,SellingType,SellingCostPara)
%% Create the grid
...
% MostBuyingX is the smallest number of log price where the amount we
% buy is most among all buyings.
MostBuyingX = find(Bbound == NumQMostBuying,1,'first');
...
for i = 1:NumQ
    ...
    if u(i) == NumX +1 %without upper bound
        ...
        if M(u(i)) > 10^5 %if the upper bound is too far away from alpha
            if i == Sbound(u(i))
                A(3*(i-1)+2,3*(i-1)+1) = 1;
                A(3*(i-1)+2,3*(i-1)+2) = 0;
                A(3*(i-1)+2,3*(i-2)+1) = -1;
    ...
    %             AggregatedSellingCost(QVec(i-1),QVec(i),Qmax);
            ...
            if i == Sbound(u(i))% (i,u(i)) is on the boundary
                A(3*(i-1)+2,3*(i-1)+1) = M(u(i));
                A(3*(i-1)+2,3*(i-1)+2) = U(u(i));
                A(3*(i-1)+2,3*(i-2)+1) = -M(u(i));
                A(3*(i-1)+2,3*(i-2)+2) = -U(u(i));
                b(3*(i-1)+2) = exp(XVec(u(i)))*dq - AggregatedSellingCost(SellingType,QVec(i-1),QVec(i),XVec(u(i)),Qmax,Qmin,NumQ,SellingCostPara);
            else
                A(3*(i-1)+2,3*(i-1)+1) = M(u(i));
                A(3*(i-1)+2,3*(i-1)+2) = U(u(i));
                A(3*(i-1)+2,3*(Sbound(u(i))-1)+1) = -M(u(i));
                A(3*(i-1)+2,3*(Sbound(u(i))-1)+2) = -U(u(i));
    ...
    if l(i) == 0 %without lower bound
        ...
        if M(l(i))>10^5
            ...
            if i == Bbound(l(i))
                A(3*(i-1)+3,3*(i-1)+3) = -M(l(i));
                A(3*(i-1)+3,3*(i-1)+2) = U(l(i));
                A(3*(i-1)+3,3*(i)+3) = M(l(i));
                A(3*(i-1)+3,3*(i)+2) = -U(l(i));
                b(3*(i-1)+3) = exp(XVec(l(i)))*dq + AggregatedBuyingCost(BuyingType,QVec(i),QVec(i+1),XVec(l(i)),Qmax,Qmin,NumQ,BuyingCostPara);
            else
                A(3*(i-1)+3,3*(i-1)+3) = -M(l(i));
                A(3*(i-1)+3,3*(i-1)+2) = U(l(i));
                A(3*(i-1)+3,3*(Bbound(l(i))-1)+3) = M(l(i));
                A(3*(i-1)+3,3*(Bbound(l(i))-1)+2) = -U(l(i));
...
for i = 1 : NumQ
    for j = MostBuyingX: NumX
        if RegionIndicater(i,j) == 0
            if  XVec(j) > alpha 
                V(i,j) = X(3*(i-1)+1) * M(j) + X(3*(i-1) +2)*U(j);
            else
                V(i,j) = X(3*(i-1)+3) * M(j) - X(3*(i-1) +2)*U(j);
            end
%         elseif j == u(i)
%             V(i,j) = X(3*(i-1)+1) *
...
%The curving back part of holding
if MostBuyingX ~=1
    XLimit = MostBuyingX - 1; % Xlimit is the limit of log price we need to consider in this region
    for j = XLimit : -1 : 1
        V(NumQMostBuying,j) = U(j) / U(MostBuyingX)* V(NumQMostBuying,MostBuyingX);
    end
    for i = NumQMostBuying - 1 : -1 : max(Bbound(1),1)
        ...
        if XLimit>0 && RegionIndicater(i,XLimit) == 0
            for j = XLimit : -1 : 1
                V(i,j) = U(j) /U(XLimit+1)* V(i,XLimit+1);
            end
        end
    end
end
